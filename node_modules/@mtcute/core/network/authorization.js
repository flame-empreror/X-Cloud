...
async function doAuthorization(connection, crypto, expiresIn) {
  ...
  function sendPlainMessage(message) {
    ...
    const writer = TlBinaryWriter.alloc(writerMap, length + 20);
  ...
  for (; ; ) {
    ...
    if (dhGen._ === "mt_dh_gen_retry") {
      const expectedHash2 = crypto.sha1(u8.concat3(newNonce, [2], authKeyAuxHash));
      if (!typed.equal(expectedHash2.subarray(4, 20), dhGen.newNonceHash2)) {
