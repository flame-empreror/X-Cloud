...
const reader$1 = require("../../tl/compat/reader.cjs");
...
function mapCompatEmojiStatus(obj) {
  switch (obj._) {
  ...
  switch (obj._) {
    ...
    case "message_layer225":
      return {
        ...
        replyMarkup: obj.replyMarkup ? mapCompatReplyMarkup(obj.replyMarkup) : void 0
      };
...
function mapCompatKeyboardButton(obj) {
  const style = "style" in obj ? obj.style : void 0;
  switch (obj._) {
...
function toInlineButton(btn) {
  const mapped = btn;
  if (mapped._ !== "keyboardInlineButton") {
    throw new errors.MtTypeAssertionError("mapCompatReplyMarkup (@ rows[*].buttons[*])", "keyboardInlineButton", mapped._);
  }
  return mapped;
}
function mapCompatReplyMarkup(obj) {
  switch (obj._) {
...
function wrapReader(reader2) {
  return (r) => mapCompatObject(reader2(r));
}
function getCombinedReaderMap() {
  const ret = {
    ...reader.__tlReaderMap
  };
  for (const [id, reader2] of utils.objectEntries(reader$1.__tlReaderMapCompat)) {
    ret[id] = wrapReader(reader2);
  }
  return ret;
}
const _combinedReaderMap = /* @__PURE__ */ getCombinedReaderMap();
function deserializeObjectWithCompat(data) {
  return tlRuntime.TlBinaryReader.deserializeObject(_combinedReaderMap, data);
}
function deserializePeersIndexWithCompat(data) {
  ...
  for (let i = 0; i < userCount; i++) {
    ...
    const obj = deserializeObjectWithCompat(io.read.exactly(bytes, len));
