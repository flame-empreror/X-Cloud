...
class UpdatesManager {
  ...
  async _fetchMissingPeers(upd, peers, fromDifference = false) {
    ...
    async function fetchEntitiesPeers(entities) {
    ...
    async function fetchRichMessagePeers(richMessage) {
        ...
        if (msg._ !== "messageService") {
          ...
          switch (msg.action._) {
            ...
            case "messageActionChatJoinedViaCommunity":
              if (!await fetchPeer(peerUtils.toggleChannelIdMark(msg.action.communityId), true)) {
                return missing;
              }
              break;
            case "messageActionChangeCommunity":
              if (msg.action.communityId !== void 0 && !await fetchPeer(peerUtils.toggleChannelIdMark(msg.action.communityId), true)) {
                return missing;
              }
              break;
